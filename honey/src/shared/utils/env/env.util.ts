...
const defaultVerificationTokenLifetime = 24 * 60 * 60 * 1000; // 24 hours converted to milliseconds
const defaultOtpLifetime = 300000; // 5 minutes converted to milliseconds
...
const mapEnv = () => {
  const parsed: IEnv = {
    ...
    auth: {
      ...
      authTokenLifetime: process.env.AUTH_TOKEN_LIFETIME || '',
      refreshTokenLifetime: process.env.REFRESH_TOKEN_LIFETIME || '',
      verificationJwtSecret: process.env.VERIFICATION_JWT_SECRET || '',
      verificationTokenLifetime: mapEnvValues.number(
        process.env.VERIFICATION_TOKEN_LIFETIME || '',
        defaultVerificationTokenLifetime,
      ),
      otpLifetime: mapEnvValues.number(process.env.OTP_LIFETIME || '', defaultOtpLifetime),
    },
    mailhog: {
